<?php /*a:2:{s:53:"/server/application/admin/view/goods_brand/lists.html";i:1765987101;s:43:"/server/application/admin/view/layout1.html";i:1765987101;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <div class="layui-card">
        ...
        <div class="layui-form layui-card-header layuiadmin-card-header-auto">
            <div class="layui-form-item">
                ...
                <div class="layui-inline">
                    <button class="layui-btn layui-btn-sm layuiadmin-btn-goods_brand <?php echo htmlentities($view_theme_color); ?>" lay-submit lay-filter="goods_brand-search">
        ...
        <div class="layui-card-body">
            <div class="layui-form-item">
                <div class="layui-inline">
                    <button class="layui-btn layui-btn-sm layuiadmin-btn-goods_brand <?php echo htmlentities($view_theme_color); ?>" data-type="add">添加品牌</button>
                </div>
            </div>
...
<script>
    layui.config({
        ...
        form.on('switch(switch-status)',function (obj) {
            ...
            like.ajax({
                url:'<?php echo url("goods_brand/switchStatus"); ?>',
        ...
        table.render({
            ...
            ,url: '<?php echo url("goods_brand/lists"); ?>' //模拟接口
            ,cols: [[
        ...
        table.on('tool(goods_brand-lists)', function(obj){
            if(obj.event === 'del'){
                ...
                layer.confirm('确定品牌:'+'<span style="color: red">'+name+'</span>', function(index){
                    like.ajax({
                        url:'<?php echo url("goods_brand/del"); ?>',
                        data:{delData:id},
                        type:"post",
                        success:function(res)
                        {
                ...
                layer.open({
                    type: 2
                    ,title: '编辑品牌'
                    ,content: '<?php echo url("goods_brand/edit"); ?>?id='+id
                    ,area: ['90%','90%']
                    ,btn: ['确定', '取消']
                    ,yes: function(index, layero){
                        ...
                        iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
                            var field = data.field;
                            like.ajax({
                                url:'<?php echo url("goods_brand/edit"); ?>',
                                data:field,
                                type:"post",
                                success:function(res)
                                {
        ...
        var active = {
            add: function(){
                layer.open({
                    type: 2
                    ,title: '添加品牌'
                    ,content: '<?php echo url("goods_brand/add"); ?>'
                    ,area: ['90%','90%']
                    ,btn: ['确定', '取消']
                    ,yes: function(index, layero){
                        var iframeWindow = window['layui-layer-iframe'+ index]
                        ...
                        iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
                            var field = data.field;
                            like.ajax({
                                url:'<?php echo url("goods_brand/add"); ?>',
                                data:field,
                                type:"post",
                                success:function(res)
                                {
