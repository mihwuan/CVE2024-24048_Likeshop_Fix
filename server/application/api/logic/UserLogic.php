...
class UserLogic extends LogicBase {
    ...
    public static function updateWechatInfo($user_id, $post)
    {
        ...
        try{
            ...
            // Kiểm tra URL avatar an toàn để ngăn SSRF
            if (!filter_var($avatar_url, FILTER_VALIDATE_URL)) {
                throw new \Exception('Invalid avatar URL');
            }

            $parsed = parse_url($avatar_url);
            if (isset($parsed['host'])) {
                $host = strtolower($parsed['host']);
                if ($host === 'localhost' || $host === '127.0.0.1' || filter_var($host, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE) === false) {
                    throw new \Exception('Unsafe avatar URL: internal/private IP not allowed');
                }
            }
            if (stripos($avatar_url, 'file://') === 0 || stripos($avatar_url, 'ftp://') === 0) {
                throw new \Exception('Unsafe protocol in avatar URL');
            }
            // Chỉ cho phép HTTP/HTTPS
            if (!preg_match('/^https?:\/\//i', $avatar_url)) {
                throw new \Exception('Only HTTP/HTTPS protocols are allowed for avatar URL');
            }
            
            // Whitelist domains (thêm các domain an toàn, ví dụ: example.com, cdn.example.com)
            $allowed_domains = ['example.com', 'cdn.example.com']; // Thay bằng domains thực tế
            $parsed = parse_url($avatar_url);
            if (isset($parsed['host'])) {
                $host = strtolower($parsed['host']);
                if (!in_array($host, $allowed_domains)) {
                    throw new \Exception('Avatar URL domain not allowed');
                }
            }
